...
-- column, per scoutsync_case_heatmap_drilldown.sql), not alongside the
-- other extract(...) columns -- CREATE OR REPLACE VIEW only allows adding
-- columns at the end of the select list; inserting one in the middle
-- shifts every later column's ordinal position, which Postgres treats as
-- an attempted rename of that column (blocked with error 42P16 -- see
-- scoutsync_rdvm_directory.sql for where this was first hit live).
-- ══════════════════════════════════════════════════════════════
